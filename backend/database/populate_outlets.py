...
def clean_outlet_data(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    if outlet_data['language'] not in valid_languages:
        logger.warning(f"Unknown language code: {outlet_data['language']} for outlet {outlet_data['name']}")
    
    # Validate URL format if provided
    if outlet_data['url'] and not outlet_data['url'].startswith(('http://', 'https://')):
        logger.warning(f"Invalid URL format for {outlet_data['name']}: {outlet_data['url']}")
        outlet_data['url'] = f"https://{outlet_data['url']}" if '.' in outlet_data['url'] else None
...
def populate_outlets_table(outlets: List[Dict[str, Any]], dry_run: bool = False) -> bool:
    ...
    if dry_run:
        ...
        for outlet in outlets[:5]:  # Show first 5 as sample
            logger.info(f"Would insert: {outlet['name']} ({outlet['language']}) - {outlet['url']}")
        if len(outlets) > 5:
...
def verify_data_integrity() -> bool:
    ...
    try:
        ...
        found_major = [name for name in major_outlets if any(name in outlet_name for outlet_name in outlet_names)]
...
def main():
    ...
    parser.add_argument(
        '--verify-only',
        action='store_true', 
        help='Only verify existing data integrity'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
