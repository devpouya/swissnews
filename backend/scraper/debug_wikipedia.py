...
def debug_wikipedia_structure():
    ...
    for header in headers:
        ...
        if span:
            print(f"Header: {header.name} - ID: '{span.get('id')}' - Text: '{span.get_text().strip()}'")
    ...
    for i, table in enumerate(tables):
        if 'wikitable' in table.get('class', []):
            ...
            if context_elements:
                ...
                for elem in reversed(context_elements[-3:]):  # Show last 3 context elements
                    print(f"    {elem}")
            ...
            for row_idx, row in enumerate(data_rows):
                ...
                if cells and any(cell for cell in cells):  # Only show non-empty rows
                    print(f"  Sample row {row_idx + 1}: {cells[:3]}...")  # First 3 columns
