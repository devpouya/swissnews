...
class ArticleExtractor:
    ...
    def extract_full_content(self, driver: webdriver.Chrome, url: str) -> ArticleContent:
        """
        ...
        try:
            ...
            article.reading_time_minutes = self._calculate_reading_time(article.word_count)
            ...
            extraction_metadata.extraction_duration_ms = int((time.time() - start_time) * 1000)
    ...
    def _extract_title(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> str:
    ...
    def _extract_subtitle(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> Optional[str]:
    ...
    def _extract_body_content(self, driver: webdriver.Chrome, article: ArticleContent, 
                            metadata: ExtractionMetadata) -> ArticleContent:
    ...
    def _extract_author(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> Optional[str]:
    ...
    def _extract_date(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> Optional[datetime]:
    ...
    def _extract_tags(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> List[str]:
    ...
    def _extract_categories(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> List[str]:
    ...
    def _extract_images(self, driver: webdriver.Chrome, base_url: str, 
                       metadata: ExtractionMetadata) -> List[ImageContent]:
            ...
            try:
                ...
                if img_elements:
                    ...
                    for img in img_elements:
                        ...
                        width = self._safe_int(img.get_attribute("width"))
                        height = self._safe_int(img.get_attribute("height"))
    ...
    def _extract_quotes(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> List[str]:
        ...
        for selector in selectors:
            ...
            try:
                elements = driver.find_elements(By.CSS_SELECTOR, selector)
                if elements:
                    quotes = [elem.text.strip() for elem in elements if elem.text.strip()]
                    if quotes:
    ...
    def _extract_highlights(self, driver: webdriver.Chrome, metadata: ExtractionMetadata) -> List[str]:
        ...
        for selector in selectors:
            ...
            try:
                elements = driver.find_elements(By.CSS_SELECTOR, selector)
                if elements:
                    highlights = [elem.text.strip() for elem in elements 
    ...
    def _find_image_caption(self, img_element: WebElement, caption_selectors: List[str]) -> Optional[str]:
    ...
    def _assess_content_quality(self, article: ArticleContent) -> float:
        ...
        if article.body_paragraphs and len(article.body_paragraphs) >= 3:
            avg_paragraph_length = sum(len(p.split()) for p in article.body_paragraphs) / len(article.body_paragraphs)
...
class ContentProcessor:
    ...
    def clean_text(self, text: str) -> str:
        ...
        if self.language == "de":
            text = re.sub(r'\[Werbung\]|\(Anzeige\)|\(Werbung\)', '', text, flags=re.IGNORECASE)
        elif self.language == "fr":
            text = re.sub(r'\[Publicité\]|\(Publicité\)', '', text, flags=re.IGNORECASE)
        elif self.language == "it":
            text = re.sub(r'\[Pubblicità\]|\(Pubblicità\)', '', text, flags=re.IGNORECASE)
    ...
    def filter_quality_images(self, images: List[ImageContent]) -> List[ImageContent]:
        ...
        for image in images:
            ...
            if any(ui_term in image.url.lower() for ui_term in 
                   ['logo', 'icon', 'button', 'banner', 'ad']):
                continue
    ...
    def _is_navigation_text(self, text: str) -> bool:
        ...
        navigation_indicators = [
            "mehr lesen", "weiterlesen", "lesen sie auch", "related",
            "teilen", "share", "kommentare", "comments",
