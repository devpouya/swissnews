...
class SwissNewsWikipediaScraper:
    def __init__(self):
        self.base_url = "https://en.wikipedia.org/wiki/List_of_newspapers_in_Switzerland"
            ...
            for field, col_index in column_map.items():
                if col_index < len(cells) and field != 'name':
                    if field == 'owner':
                        ...
                        outlet['occurrence'] = self.clean_text(cells[col_index].get_text())
    ...
    def scrape_all_languages(self) -> List[Dict]:
        ...
        for table_idx, language, status in table_language_map:
            if table_idx < len(tables):
                try:
                    logger.info(f"Processing table {table_idx}: {language} {status} outlets")
                    ...
                    logger.error(f"Error processing table {table_idx} ({language} {status}): {e}")
                    continue
            else:
                logger.warning(f"Table {table_idx} not found (expected {language} {status})")
